...
describe('<SearchResult />', () => {
  it('renders the item label', () => {
    ...
    expect(wrapper.find(Highlight).at(0).prop('fallback')).toBe(item.label);
  });

  it('renders a description', () => {
    const wrapper = shallow(<SearchResult item={item} formattedParents="" />);

    expect(wrapper.find(Highlight).at(1).prop('fallback')).toBe(item.description);
  });
  ...
  it('renders a keyword match', () => {
    ...
    expect(wrapper.find(Highlight).at(2).prop('match')).toBe(match);
  });
});
